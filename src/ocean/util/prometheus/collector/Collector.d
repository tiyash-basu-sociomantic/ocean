...
/*******************************************************************************
    ...
    multiple labels, via the following methods:

    1.  For collecting stats from a struct or class:
        `void collect ( ValuesT ) ( ValuesT values )`

    2.  For collecting stats from a struct or class, and with one label:
        `void collect ( istring LabelName, ValuesT, LabelT ) (
            ValuesT values, LabelT label_val )`

    3.  For collecting stats from a struct or class, along with a set of labels
        from a struct or class:
        `void collect ( ValuesT, LabelsT ) ( ValuesT values, LabelsT labels )`

    Once the desired stats have been collected, the accumulated response
    string can be collected using the `cstring getCollection ( )` method.

    Additionally, the `void reset ( )` method can be used to reset stat
